...
public class Interpreter {
    ...
    public boolean execute(List<String> script) {
        try {
            for (String instruction : script) {
                ...
                if (traceEnabled) System.out.println("Ejecutando: " + instruction);
                ...
                if (traceEnabled) stack.printStack();
            }
    ...
    private void handleOpCode(String op) {
        switch (op) {
            ...
            case "OP_1": case "OP_TRUE": stack.push(new byte[]{1}); break;
            case "OP_DUP": stack.push(stack.peek().clone()); break;
            case "OP_DROP": stack.pop(); break;
            case "OP_ADD": performArithmetic((a, b) -> a + b); break;
            case "OP_SUB": performArithmetic((a, b) -> a - b); break;
            case "OP_EQUAL": executeEqual(); break;
            case "OP_EQUALVERIFY": 
                executeEqual(); 
                if (!isTrue(stack.pop())) throw new RuntimeException("OP_EQUALVERIFY falló");
                break;
            case "OP_IF": conditionStack.push(isTrue(stack.pop())); break;
            case "OP_ELSE":
                if (conditionStack.isEmpty()) throw new RuntimeException("OP_ELSE sin OP_IF");
                conditionStack.push(!conditionStack.pop());
                break;
            case "OP_ENDIF":
                if (conditionStack.isEmpty()) throw new RuntimeException("OP_ENDIF sin OP_IF");
                conditionStack.pop();
                break;
            case "OP_HASH160":
                ...
                break;
            case "OP_CHECKSIG":
                stack.pop(); stack.pop(); // Simulación de remoción de pubKey y firma
    ...
    private int bytesToInt(byte[] data) {
        try { return Integer.parseInt(new String(data)); }
        catch (Exception e) { return data[0]; }
    }
}